...
int FCT_TEST_wifi_2Ghz(int nItfe, char	*chMessageErreur )
{
	...
	sprintf(pythonNEW,"app.exe %s/logOutput.txt logOutput.txt 2G", pytlog) ;
	...
	if(gSlot[nItfe].BNFTnteg.gchFlagTraca)
	       ...
	       //Status= 	MES_EnregistrerTableauV2 (ptrMesures,nItfe, fichier, chMessageErreur ) ;
	            ...
	            Status  = MES_EnregistrerTableauV2Log  ( ptrMesures,Pathlogoutput,nItfe ,fichier, chMessageErreur  );		
			}
	...
	if ( Status <0)
	{
		...
		if( Status < 0 )
		{
			...
			sprintf( cheminlogscript,"%s\\Log\\%s_%s_%s_%s_2Ghz.log" ,VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0),"UNKOWN",ptrVariablesTraca->CieSn,chDate,chHeureMili);
			...
			//system(python);
			 ...
			 {
				Status=-1; 
		  		//FCT_AfficheInfo (nItfe, IHM_MODE_TRACE_TXTBOX|IHM_MODE_TRACE_LOG, 999,"----->TEST WIFI 2Ghz FAILED");
				//strcpy(chMessageErreur,Msg_Erreur);
				//strcpy(chMessageErreur,"TEST WIFI 2Ghz FAILLED !!!!");
				sprintf( cheminlogscript,"%s\\Log\\%s_%s_%s_%s_2Ghz.log" ,VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0),"FAILED",ptrVariablesTraca->CieSn,chDate,chHeureMili);
	            sprintf( logfolder,"%s\\Log" , VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0) ) ; 
			 ...
			 sprintf( cheminlogscript,"%s\\Log\\%s_%s_%s_%s_2Ghz.log" ,VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0),"FAILED",ptrVariablesTraca->CieSn,chDate,chHeureMili);
	         sprintf( logfolder,"%s\\Log" , VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0) ) ; 
			sprintf(python,"app.exe %s/logOutput.txt logOutput.txt 2G", logfolder,cheminlogscript) ;
	...
	sprintf(cccommand , "mainn.exe %s", GG);
	...
	else
	 {
		...
		sprintf( cheminlogscript,"%s\\Log\\%s_%s_%s_%s_2Ghz.log" ,VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0),"PASSED",ptrVariablesTraca->CieSn,chDate,chHeureMili);
	    sprintf( logfolder,"%s\\Log" , VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0) ) ; 
	...
	char pytlog1 [1024]; 
	sprintf(pytlog,"%s\\Log" , VAR_GET_STRING(ptrVariablesConf,VAR_CONF_GTW_BWC_PATH_SCRIPT_2GHZ,0)) ;
	sprintf(pythonNEW1,"app.exe %s/logOutput.txt logOutput.txt 2G", cheminlogarchivedate,cheminlogscript) ;
	system(pythonNEW1);
	...
	//sprintf(python,"app.exe %s %s", cheminlogarchivedate,cheminlogscript) ;
	//system(python);
	Sleep(200);
...
int FCT_CMD_PingGateWay (int nItfe, char *messagErreur)
{
   ...
   // system("C:\\Users\\G558412\\Desktop\\Tftpd64\\tftpd64.exe");
	//LaunchExecutable("C:\\Tftpd64\\tftpd64.exe");
