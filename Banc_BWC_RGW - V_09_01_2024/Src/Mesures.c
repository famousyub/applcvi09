...
TableauMesures = (char**) malloc( sizeof(char*) *  400 * CHAMPS_TAB);
		...
		while(  !feof(f)   )
		{
			...
			fscanf(f  , "%s ; %f ; %s ; %s ; %s ; %s ; %s ; %s ; %c" ,nom , &m ,_date, a, minb, maxc , step ,readeruni,&_com );
			...
			max = atof(maxc ) ;
			min = atof(minb) ;
			if (isalnum(_m)   &&   isalnum(max)   &&  isalnum(min) )
			{
				if (_m <=  max  && _m >= min  )
				{
					valeur = 1;
...
SCPROD_LogParametricDataTableau (nItfe, TableauMesures, (idxMesure-1), CHAMPS_TAB, "test mE");
